...
function scheduleMonster(){const ticket=epoch;timer=setTimeout(async()=>{
 ...
 try{
  ...
  const wins=choices.filter(m=>m.winning);let pool=choices;const chance={perfect:1,tactical:.82,friendly:.45}[$('difficulty').value];if(wins.length&&Math.random()<chance)pool=wins;
  const m=pool.length?pool[Math.floor(Math.random()*pool.length)]:{x:0,y:0};
