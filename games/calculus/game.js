...
function render(){for(const [kind,items] of [['functions',state.functions],['derivatives',state.derivatives]]){$(kind).innerHTML=items.map((item,i)=>{const isF=kind==='functions',done=state.matches.has(item.id),selected=isF&&state.selected===item.id,label=isF?String.fromCharCode(65+i):String(i+1),partner=isF?state.derivatives.findIndex(d=>d.id===item.id)+1:String.fromCharCode(65+state.functions.findIndex(f=>f.id===item.id));return `<button class="graph-card ${done?'matched':''} ${selected?'selected':''}" data-kind="${kind}" data-id="${item.id}" ${done?'disabled':''} ${isF?`aria-pressed="${selected}"`:''} aria-label="${isF?'Function':'Derivative'} ${label}${done?`, matched with ${partner}`:''}"><span class="card-label"><span>${isF?'Function':'Derivative'} ${label}</span><span class="pair-label">${done?'♥ '+partner:selected?'Selected':''}</span></span>${graph(isF?item.f:item.d,isF?'#4a47ce':'#ce5282',kind+i)}</button>`}).join('');} $('completion-title').textContent=state.round===3?'Matchmaking mastered!':'Perfect chemistry!';$('completion-copy').textContent=state.round===3?'You completed all three rounds. Cupid can retire.':state.round===1?'Ready for a higher difficulty? Round 2 allows only two mistakes. A third mistake ends the attempt.':'The final round raises the difficulty again: no mistakes allowed. Every match must be perfect.';$('again').textContent=state.round===3?'Play again ↻':'Next round ↗';if(state.matches.size===5&&!$('completion').open)$('completion').showModal();}
function mistakeLimit(){return state.round===1?Infinity:state.round===2?2:0;}
function updateBudget(){const limit=mistakeLimit();$('mistake-budget').textContent=limit===Infinity?'Unlimited mistakes — get to know the graphs.':limit===0?'No mistakes allowed — make every match count.':`${Math.max(0,limit-state.mistakes)} of 2 mistakes remaining`;}
function dealRound(){if($('completion').open)$('completion').close();state.failed=false;state.functions=makeFunctions();state.derivatives=shuffle(state.functions);state.selected=null;state.matches.clear();state.mistakes=0;$('round').textContent=`ROUND ${state.round} OF 3${state.round===1?' · PRACTICE':''}`;$('status').textContent='';render();updateBudget();}
...
function failedRound(){state.failed=true;$('loss-copy').textContent=matchmakingJokes[Math.floor(Math.random()*matchmakingJokes.length)];$('retry-copy').textContent=`Try round ${state.round} again with five new pairs.`;$('loss-dialog').showModal();}
...
function choose(kind,id){if(state.failed||state.matches.has(id))return;if(kind==='functions'){state.selected=id;render();$('status').textContent='Now choose the derivative for Function '+String.fromCharCode(65+id)+'.';}else{if(state.selected===null){$('status').textContent='Pick a function from the first row to begin.';return;}if(id===state.selected){state.matches.set(id,true);state.selected=null;render();$('status').textContent=state.matches.size===5?'All five matched! You’ve got a great eye for slopes.':'That’s a match! Choose another function.';}else{state.mistakes++;updateBudget();if(state.mistakes>mistakeLimit()){failedRound();return;}$('status').textContent='Not quite a match. Compare the slope and try again — you’ve got this.';const btn=document.querySelector(`[data-kind="derivatives"][data-id="${id}"]`);btn.classList.remove('wrong');void btn.offsetWidth;btn.classList.add('wrong');}}}
